...
A write operation that touches persistence: an application-layer use case owns the
transaction, talks to infrastructure only through a `Protocol` port, and commits once at
the end — never call a repository or a session from a router directly.
...
class CreateFoo:
    def __init__(self, uow: FooUnitOfWork) -> None:
        self._uow = uow
...
def test_create_foo_adds_and_commits() -> None:
    ...
    use_case = CreateFoo(uow)

    result = asyncio.run(use_case(CreateFooCommand(name="widget")))

    assert result.name == "widget"
    assert repo.added == [(result.id, "widget")]
    assert uow.committed is True
```

## Checklist

- Ports (`FooRepository`, `FooUnitOfWork`) are `Protocol`s, never ABCs — infrastructure
  implements them by shape, not inheritance. Command and result are frozen `dataclass`es.
- The use case calls `uow.commit()` exactly once, inside the `async with` block; a
  repository never calls `commit()`, only `flush()`.
- Production placement is `app/modules/<module>/application/`; the marker file names
  above are proof-run paths only, not the production layout.
- The test uses an in-memory fake repository and fake unit of work, driven with
  `asyncio.run`, and asserts literal values, never values echoed back from the use case.
