...
flowchart TD
  ...
  review -->|есть открытые треды| answer["5. Автор отвечает в каждом треде,<br/>Re-request review"]
  answer --> recheck["6. Ревьюер перепроверяет,<br/>закрывает обработанные треды"]
  recheck -->|открытые треды остались| answer
...
| Тред закрывает только ревьюер | договорённость: GitHub разрешает закрыть тред автору PR и любому с правом записи, а права записи есть у всей команды |
